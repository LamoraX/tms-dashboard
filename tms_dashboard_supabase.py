...
def get_next_session_number(patient_id):
    c = conn.cursor()
    c.execute("SELECT MAX(session_number) FROM tms_sessions WHERE patient_id = %s", (patient_id,))
    result = c.fetchone()[0]
    return (result + 1) if result else 1

def is_holiday(date):
    c = conn.cursor()
    c.execute("SELECT * FROM holidays WHERE holiday_date = %s AND skip_enabled = 1", (date,))
...
if page == "📊 Daily Dashboard":
    ...
    with col1:
        ...
        if st.button("Save Staff Assignment"):
            ...
            c.execute("""UPDATE daily_slots 
                        SET sr_name = %s, jr1_name = %s, jr2_name = %s
                        WHERE slot_date = %s""",
    ...
    with col2:
        ...
        # Count today's slots
        query = "SELECT COUNT(*) FROM daily_slots WHERE slot_date = %s"
        c = conn.cursor()
    ...
    if not patients_df.empty:
        ...
        if st.button("Remove Selected Patient", type="primary"):
            if password == "123":
                ...
                c.execute("DELETE FROM tms_sessions WHERE patient_id = %s", (patient_id,))
                c.execute("DELETE FROM daily_slots WHERE session_id IN (SELECT id FROM tms_sessions WHERE patient_id = %s)", (patient_id,))
                conn.commit()
...
# PAGE 3: SLOT MANAGEMENT
    ...
    if patients_df.empty:
        ...
        if st.button("Create Slots", type="primary") and protocol_id:
            ...
            c.execute("SELECT session_duration FROM protocol_library WHERE id = %s", (protocol_id,))
        ...
        if df_today.empty:
            ...
            if st.button("Complete Session", type="primary"):
                ...
                c.execute("""UPDATE tms_sessions 
                           SET protocol_id = %s, target_laterality = %s, target_region = %s,
                               coord_left_x = %s, coord_left_y = %s, coord_right_x = %s, coord_right_y = %s,
                               rmt_left = %s, rmt_right = %s,
                               intensity_percent_left = %s, intensity_percent_right = %s,
                               intensity_output_left = %s, intensity_output_right = %s,
                               coil_type = %s, side_effects = %s, remarks = %s, status = 'Completed'
                           WHERE id = %s""",
    ...
    with tab1:
        ...
        if not protocols_df.empty:
            ...
            if st.button("Delete Selected Protocol", type="primary"):
                c = conn.cursor()
                c.execute("DELETE FROM protocol_library WHERE protocol_name = %s", (delete_protocol,))
                conn.commit()
