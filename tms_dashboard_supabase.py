...
if page == "📊 Daily Dashboard":
    ...
    results = execute_query(
        """SELECT p.name as patient_name, ts.session_number, pl.protocol_name, 
                  ...
                  ds.scheduled_time, ds.status, ds.slot_duration
           FROM daily_slots ds
    ...
    if results:
        df = pd.DataFrame(results, columns=['Patient', 'Session', 'Protocol', 'Target', 'Time', 'Status', 'Duration'])
...
# ==================== PAGE 2: PATIENT REFERRAL ====================
    ...
    st.markdown("### 🗑️ Remove Patient (Admin)")
        ...
        if st.button("Remove Selected Patient", type="primary"):
            if password == "admin123":
                ...
                if execute_update("DELETE FROM patients WHERE id = %s", (patient_id,)):
                    st.success("✅ Patient and associated records deleted!")
            else:
...
# Footer
st.sidebar.markdown("---")
st.sidebar.info("💡 TMS Integration Dashboard v2.0 by Dr Aromal")
